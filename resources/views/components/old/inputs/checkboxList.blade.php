    ...
    <div class="{!! $_X['input_size'] !!}">
        <div class="kt-checkbox-list">
            ...
            <label class="kt-checkbox">
                <input name="{!! $_X['name'] !!}" type="checkbox" class="form-control {!! $_X['class'] !!}" value="{!! $_X['values']($i, $option) !!}" {!! $_X['attr'] !!} @if( $attr=$_X['option_attr']($i, $option) ) {!! $attr !!} @endif @if( $_X['select']($i, $option, $_X['value']) ) checked="" @endif @if( $_X['disable']($i, $option, $_X['value']) ) disabled="" @endif {!! $_X['readonly'] ? 'readonly=""' : '' !!} {!! $_X['disabled'] ? 'disabled=""' : '' !!}>
                {!! $_X['text']($i, $option) !!}
                <span></span>
            </label>
            @endforeach
...
<div class="form-group ">
    ...
    <div class="kt-checkbox-list {!! $_X['list_class'] !!}">
        @foreach($_X['options'] as $i => $option)
        <label class="kt-checkbox  {!! $_X['lable_class'] !!}">
            <input name="{!! $_X['name'] !!}" type="checkbox" class="form-control {!! $_X['class'] !!}" value="{!! $_X['values']($i, $option) !!}" {!! $_X['attr'] !!} @if( $attr=$_X['option_attr']($i, $option) ) {!! $attr !!} @endif @if( $_X['select']($i, $option, $_X['value']) ) checked="" @endif @if( $_X['disable']($i, $option, $_X['value']) ) disabled="" @endif {!! $_X['readonly'] ? 'readonly=""' : '' !!} {!! $_X['disabled'] ? 'disabled=""' : '' !!}>
            {!! $_X['text']($i, $option) !!}
            <span></span>
        </label>
